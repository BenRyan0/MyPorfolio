...
const Laptop = ({ scale, position, rotationX, rotationY }) => {
  ...
  return (
    ...
    >
      <primitive object={scene} position={[-0.4, 0, 0.3]} />
    </animated.mesh>
  );
};
const LaptopCanvas = ({ scrollContainer }) => {
  ...
  const defaultRotation = [0, 0, 6.33]; // your original rotation
...
const [isInactive, setIsInactive] = useState(false);
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      if (window.innerWidth < 768) {
            setScale([.47, .47, 0.2]);
        setPosition([-0.5, 0.4, -0.9]); // Slight vertical offset, centered horizontally
      } else if (window.innerWidth < 1024) {
          setScale([1.2, 1.2, 0.59]);
        setPosition([-0.2, -1.4, -0.9]); // Slight vertical offset, centered horizontally
      } else if (window.innerWidth < 1280) {
         setScale([1.2, 1.2, 0.59]);
        setPosition([-0.3, -1.4, -0.9]);// Slight vertical offset, centered horizontally
      } else if (window.innerWidth < 1536) {
        setScale([1.2, 1.2, 0.59]);
        setPosition([-0.6, -1.4, -0.9]); // Slight vertical offset, centered horizontally
      } else {
          setScale([1.2, 1.2, 0.59]);
        setPosition([-0.2, -1.4, -0.9]);  // Slight vertical offset, centered horizontally
      }
  ...
  setIsInactive(false);
    ...
    const clampedX = clamp(rawX, -0.1, 0.1);
    const clampedY = clamp(rawY, -0.1, 0.1);

    setMouseX(clampedX);
    setMouseY(clampedY);
  };

const computedRotationX = isInactive ? defaultRotation[0] : rotationX + mouseY * 1.2;
const computedRotationY = isInactive ? defaultRotation[1] : rotationY + mouseX * 1.5;

  return (
    <Canvas
      className={`w-full h-screen z-10 mt-10 rotate-3`}
